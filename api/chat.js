...
export default async function handler(req,res){
 ...
 try{
  ...
  const adaptiveTemperature=mode==='code'||mode==='maths'?0.10:complexitySignals>=3?0.10:0.12,adaptiveMaxTokens=isDeepTask?20000:complexitySignals>=1?16000:12000;const autonomousStrategy=isDeepTask?'plan → agent spécialisé → génération → vérification → correction éventuelle':'comprendre → choisir la meilleure stratégie → répondre → contrôle final';
  ...
  const systemPrompt=[
   'Tu es Nexus AI V3.7, le moteur central de Nexus Core, un assistant francophone généraliste qui orchestre des agents spécialisés.',
   ...
   'Tu es maintenant Nexus AI V3.7. Ne prétends jamais avoir utilisé un outil ou vérifié une information externe si ce n’est pas réellement le cas.'
  ].filter(Boolean).join('\n\n');
  ...
  return res.status(200).json({text,version:'3.7',core:buildFutureEngineContract(),autonomousCore:{enabled:true,strategy:autonomousStrategy,replanned:isDeepTask},mode,mission,hasImage,hasDocument,provider:routed.provider,model:routed.model,fallbackUsed:Boolean(routed.fallbackUsed),fallbackReason:routed.fallbackReason||null,contextAdvisor:{used:contextSupport.needed,ambiguityScore:contextSupport.ambiguityScore},specializedAgent:{id:specializedAgent.id,name:specializedAgent.name},verification});
 }catch(error){console.error('Nexus AI V3.7 chat error:',error);return res.status(error.status===503?503:500).json({error:error.message||'Erreur du moteur IA.'});}
}
