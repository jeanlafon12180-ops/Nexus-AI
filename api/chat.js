...
import { buildContextSupport, buildVerificationPrompt, buildCorrectionPrompt, isUsefulReview } from './nexus-assist.js';
...
export default async function handler(req,res){
 ...
 try{
  ...
  const systemPrompt=[
   'Tu es Nexus AI V3.4, le moteur central de Nexus Core, un assistant francophone généraliste de très haut niveau.',
   ...
   contextSupport.support?'NEXUS CONTEXT ADVISOR V3.4 :\n'+contextSupport.support:'',mission.isMission?'MISSION ENGINE V3.4 : plan de mission : '+missionPlan.steps.join(' → ')+'. Utilise ce plan comme cadre de travail, puis vérifie le résultat avant de conclure.':'',
   ...
   'Tu es maintenant Nexus AI V3.4. Ne prétends jamais avoir utilisé un outil ou vérifié une information externe si ce n’est pas réellement le cas.'
  ].filter(Boolean).join('\n\n');
  ...
  return res.status(200).json({text,version:'3.4',core:buildFutureEngineContract(),mode,mission,hasImage,hasDocument,provider:routed.provider,model:routed.model,fallbackUsed:Boolean(routed.fallbackUsed),fallbackReason:routed.fallbackReason||null,contextAdvisor:{used:contextSupport.needed,ambiguityScore:contextSupport.ambiguityScore},verification});
 }catch(error){console.error('Nexus AI V3.4 chat error:',error);return res.status(error.status===503?503:500).json({error:error.message||'Erreur du moteur IA.'});}
}
