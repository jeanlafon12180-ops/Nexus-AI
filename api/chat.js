...
export default async function handler(req,res){
 ...
 try{
  ...
  const systemPrompt=[
   'Tu es Nexus AI V3.3, le moteur central de Nexus Core, un assistant francophone généraliste de très haut niveau.',
   ...
   mission.isMission?'MISSION ENGINE V3.3 : plan de mission : '+missionPlan.steps.join(' → ')+'. Utilise ce plan comme cadre de travail, puis vérifie le résultat avant de conclure.':'',
   ...
   'Tu es maintenant Nexus AI V3.3. Ne prétends jamais avoir utilisé un outil ou vérifié une information externe si ce n’est pas réellement le cas.'
  ].filter(Boolean).join('\n\n');
  ...
  const routed=await routeModel({messages:[{role:'system',content:systemPrompt},...historyMessages,{role:'user',content:userContent}],temperature:adaptiveTemperature,maxTokens:adaptiveMaxTokens});
  const text=routed.data?.choices?.[0]?.message?.content;if(!text)throw new Error('Le moteur IA n’a renvoyé aucune réponse.');
  mission=mission.isMission===false?mission:completeMissionExecution(mission);
  return res.status(200).json({text,version:'3.3',core:buildFutureEngineContract(),mode,mission,hasImage,hasDocument,provider:routed.provider,model:routed.model,fallbackUsed:Boolean(routed.fallbackUsed),fallbackReason:routed.fallbackReason||null});
 }catch(error){console.error('Nexus AI V3.3 chat error:',error);return res.status(error.status===503?503:500).json({error:error.message||'Erreur du moteur IA.'});}
}
